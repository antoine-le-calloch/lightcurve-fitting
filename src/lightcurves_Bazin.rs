...
impl CostFunction for BazinCost {
    type Param = Vec<f64>;   // parameters in log-space
    type Output = f64;       // chi2 value
    ...
    fn cost(&self, p: &Self::Param) -> Result<Self::Output, Error> {
        ...
        let a        = log_a.exp();
        let tau_rise = log_tau_r.exp();
        let tau_fall = log_tau_f.exp();
        ...
        for i in 0..self.times.len() {
            let model = bazin_flux(a, b, t0, tau_rise, tau_fall, self.times[i]);
        ...
        // Bazin **only** requires tau_rise < tau_fall for a true maximum.
        // If you want to enforce that softly:
        let x = tau_rise / tau_fall;
        if x >= 1.0 {
           chi2 += 1e6 * (x - 1.0);
        }
        // ───────────────────────────────────────
...
/// PSO bounds for the Bazin light-curve model.
pub fn pso_bounds_bazin() -> (Vec<f64>, Vec<f64>) {
    let lower = vec![
        -10.0,  // log(A) min  ~ exp(-10) = 4.5e-5
        -1.0,   // B min
        -50.0,  // t0 min
        -2.3,   // log(tau_rise) ~ exp(-2.3) ≈ 0.1 days
        -0.7,   // log(tau_fall) ~ exp(-0.7) ≈ 0.5 days
    ];

    let upper = vec![
        5.0,    // log(A) max ~ exp(5) ≈ 148
        1.0,    // B max
        50.0,   // t0 max
        4.6,    // log(tau_rise) ~ exp(4.6) ≈ 100 days
        5.7,    // log(tau_fall) ~ exp(5.7) ≈ 300 days
    ];
...
pub fn analyze_photometry(
    ...
    // Plot using plotters
    let root = BitMapBackend::new("bazin_fit.png", (800, 600)).into_drawing_area();
    root.fill(&WHITE);
