...
fn compute_rise_rate(times: &[f64], mags: &[f64]) -> f64 { /* same as original */
...
fn compute_decay_rate(times: &[f64], mags: &[f64]) -> f64 { /* same as original */
    if times.len() < 2 { return f64::NAN; }
    let n_late = (times.len() as f64 * 0.25).ceil() as usize; let n_late = n_late.max(2).min(times.len()); let start_idx = times.len() - n_late;
