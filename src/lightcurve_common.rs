...
pub fn read_ztf_lightcurve_f32(path: &str, convert_to_mag: bool) -> Result<HashMap<String, BandDataf32>, Box<dyn std::error::Error + Send + Sync>> {
    let contents = fs::read_to_string(path)?;
    let mut bands: HashMap<String, BandDataf32> = HashMap::new();
    ...
    for ((filter, _), (flux, mjd, flux_err)) in epoch_best.iter() {
        ...
        let band = bands.entry(filter.clone()).or_insert_with(|| BandDataf32 { times: Vec::new(), mags: Vec::new(), errors: Vec::new() });
        band.times.push(delta_t as f32);
        band.mags.push(value as f32);
        band.errors.push(error as f32);
    }
